...
需要用户做**选择**时（门禁确认、批准 patch、分类纠正、挑选条目等），**优先调用 `AskUserQuestion` 工具**让用户在预设选项里勾选；只有在该工具不可用时才退化为让用户自由打字回复。

- 超过 4 个选项或超过 4 个问题 → 分批调用
- 不要手动提供 "Other"，该选项由 `AskUserQuestion` 自动附加
- 需要用户补充原因/自由文案时，让他们选 "Other" 并填写
...
| 2 | 解析 `[必修]/[建议]/[信息]` 标签，门禁判断 | 是（有 `[必修]`/`[建议]`/`[信息]` 都要停） |
| 3 | 生成中文 MR 标题（≤30 字） | 否 |
...
**优先用 `AskUserQuestion`**：对每条 `[建议]`/`[信息]`（≤4 条一批）构造问题，选项固定：

- "修复" → 用户去改代码，结束本轮，让用户改完重新触发 `/gitlab-mr`
- "不处理" → 等用户选 "Other" 写明原因（或直接选"不处理"表示无特殊理由）

用户选 "不处理" 后，将原因（若有）追加到报告对应条目末尾：`——作者回复：<原因>`；继续 Phase 3。任何一条选 "修复" → 硬停等修代码。
...
请回复：
  · 修复  → 改代码后重新触发 /gitlab-mr
  · 不处理 + 原因 → 原因附到报告，继续流程
```

### 只有 `[需判断]` 或完全无标签 → 无缝进入 Phase 3

...
7. For another project, run `bin/agentsync` to establish the instruction pair,
   add missing portable-skill aliases, and verify both directions in one pass.
8. Validate every new or changed skill with the active skill-creator package's
   `scripts/quick_validate.py`, and run `test/agentsync.sh` when topology or
...
- Do not prune unknown skills or links. `agentsync` may create missing links but
  must refuse real directories, foreign links, and orphaned reverse entries.
- `bin/agentsync` checks another project's root instruction pair and project
